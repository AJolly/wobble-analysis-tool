...
const SleepAnalyzer = () => {
  ...
  const [ignoreFirstNinety, setIgnoreFirstNinety] = useState(false);

  const getScore = (session, key) => {
    if (ignoreFirstNinety && session[`trimmed_${key}`] != null) {
      return session[`trimmed_${key}`];
    }
    return session[key];
  };

  const getDuration = (session) => {
    if (ignoreFirstNinety && session.trimmed_durationMinutes != null) {
      return session.trimmed_durationMinutes;
    }
  ...
  const consolidateByNight = (sessionResults) => {
    ...
    // Filter out sessions that have no trimmed data when in trimmed mode
    const validSessions = ignoreFirstNinety
      ? sessionResults.filter(s => s.trimmed_flScore != null)
      : sessionResults;

    if (validSessions.length === 0) return [];

    // Sort by date first
    const sorted = [...validSessions].sort((a, b) => a.date - b.date);
    ...
    return nights.map(night => {
      const totalDuration = night.sessions.reduce((sum, s) => sum + getDuration(s), 0);
      const isNap = totalDuration < 240; // Less than 4 hours = nap

      return {
        date: night.date,
        filename: night.filenames.join(', '),
        flScore: night.sessions.reduce((sum, s) => sum + (getScore(s, 'flScore') * getDuration(s)), 0) / totalDuration,
        periodicityIndex: night.sessions.reduce((sum, s) => sum + (getScore(s, 'periodicityIndex') * getDuration(s)), 0) / totalDuration,
        regularityScore: night.sessions.reduce((sum, s) => sum + (getScore(s, 'regularityScore') * getDuration(s)), 0) / totalDuration,
        eai: night.sessions.reduce((sum, s) => sum + (getScore(s, 'eai') * getDuration(s)), 0) / totalDuration,
  ...
  const handleFileUpload = async (event) => {
    ...
    for (let i = 0; i < validFiles.length; i++) {
      try {
        ...
        // Compute trimmed scores (skip first 1.5 hours)
        const samplesToSkip = Math.floor(90 * 60 * samplingRate);
        let trimmedScores = {};
        if (flowData.length > samplesToSkip) {
          const trimmedFlow = flowData.slice(samplesToSkip);
          const trimmedDuration = durationMinutes - 90;
          const { flScore: tFL, breaths: tBreaths } = analyzeFlowLimitation(trimmedFlow, samplingRate);
          const tEai = estimateArousals(tBreaths, trimmedFlow, samplingRate, trimmedDuration * 60);
          const tMinuteVent = calculateMinuteVent(trimmedFlow, samplingRate);
          const { periodicityIndex: tPI, regularityScore: tRS } = analyzeNight(tMinuteVent);
          trimmedScores = {
            trimmed_flScore: tFL,
            trimmed_periodicityIndex: tPI,
            trimmed_regularityScore: tRS,
            trimmed_eai: tEai,
            trimmed_durationMinutes: trimmedDuration
          };
        }
        ...
        {results.length > 0 && (
          <>
            <div className="flex items-center gap-3 mb-4">
              <button
                onClick={() => setIgnoreFirstNinety(!ignoreFirstNinety)}
                className={`text-sm font-semibold py-2 px-4 rounded transition-colors border ${
                  ignoreFirstNinety
                    ? 'bg-amber-600 hover:bg-amber-700 text-white border-amber-500'
                    : 'bg-white/10 hover:bg-white/20 text-white border-white/20'
                }`}
              >
                {ignoreFirstNinety ? 'First 1.5h Ignored' : 'Ignore First 1.5h'}
              </button>
              {ignoreFirstNinety && (
                <span className="text-amber-200 text-xs">
                  Scores exclude first 90 minutes of each session
                </span>
              )}
            </div>

            <ResultsTable results={results} rawSessions={rawSessions} ignoreFirstNinety={ignoreFirstNinety} />
